<script setup>
/* -----------------------------------------------------------
   SudokuRunner.vue  –  Ejecución de un solver en el navegador
------------------------------------------------------------*/
import { ref, onMounted } from 'vue'

/* 1. Código Python (tal cual lo pegaste).
      Lo convertimos en una template-string para pasarlo
      a Pyodide.  */
const PYTHON_SOURCE = `# ----------------  Solver de Sudokus  ----------------
${String.raw`
def leer_tablero():
    """
...
def resolver_cadena(cadena):
    """
    cadena: str de 81 caracteres (0 ó . para huecos)
    Devuelve la salida textual que normalmente imprime main().
    Evita los input() interactivos.
    """
    import io, sys, contextlib, textwrap
    import builtins

    # Redirigir stdout a StringIO para capturar prints
    capt = io.StringIO()
    with contextlib.redirect_stdout(capt):
        # Armar tablero a partir de la cadena
        tablero = [[int(c) if c != '.' else 0
                    for c in cadena[i*9:(i+1)*9]] for i in range(9)]
        imprimir_tablero(tablero)

        cand = calcular_candidatos(tablero)
        imprimir_candidatos(cand, 'Candidatos Iniciales')

        tablero_sol, it, prof, t = resolver_sudoku_fuerza_bruta(tablero)
        if tablero_sol:
            print(f'¡Resuelto en {t:.4f} s – {it} iteraciones!')
            imprimir_tablero(tablero_sol)
        else:
...
/* 2. Estados reactivos */
const sudokuStr = ref('' /* el usuario pegará 81 dígitos o puntos */)
const output    = ref('')
const running   = ref(false)
let pyodide     // instancia compartida (singleton)

/* 3. Carga perezosa de Pyodide solo la primera vez */
async function ensurePyodide () {
  if (pyodide) return pyodide
  // Carga desde CDN oficial
  pyodide = await window.loadPyodide({
    indexURL: 'https://cdn.jsdelivr.net/pyodide/v0.27.6/'
  })
  await pyodide.loadPackage([])      // por si se requieren stdlib extras
  await pyodide.runPythonAsync(PYTHON_SOURCE) // definimos solver
  return pyodide
}

/* 4. Ejecutar solver */
async function runSolver () {
  if (sudokuStr.value.trim().length !== 81) {
    output.value = '⚠️  Introduce exactamente 81 caracteres (0-9 o .)'
    return
  }
  running.value = true
  output.value  = '⏳ Ejecutando…'

  const py = await ensurePyodide()
  // Pasar la cadena a Python y llamar a resolver_cadena
  const code = `
from js import sudoku_input
result = resolver_cadena(sudoku_input)
`
  // Mandamos la variable a Pyodide
  py.globals.set('sudoku_input', sudokuStr.value.trim())
  await py.runPythonAsync(code)
  output.value = py.globals.get('result')        // recogemos resultado
  running.value = false
}

/* 5. Preparar resaltado al montar (solo decorativo) */
onMounted(() => {
  import('prismjs')            // VitePress ya trae Prism
}) 
</script>

<template>
  <ClientOnly>
    <div class="sudoku-runner">
      <!-- Área de entrada -->
      <textarea v-model="sudokuStr"
                spellcheck="false"
                rows="3"
                placeholder="Pega aquí 81 caracteres, usa 0 o . para huecos">
      </textarea>

      <!-- Botón -->
      <button class="vp-button" @click="runSolver" :disabled="running">
        {{ running ? 'Ejecutando…' : 'Resolver' }}
      </button>
      ...
      <details>
        ...
        <pre class="language-python"><code>{{ PYTHON_SOURCE }}</code></pre>
      </details>
    </div>
  </ClientOnly>
</template>
