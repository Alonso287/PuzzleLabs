def leer_tablero():
    ...
    """
    opcion = input(
        "¿Cómo deseas introducir el tablero de Sudoku?\n1. Por filas\n2. Como una cadena única\nOpción: "
    ).strip()
    ...
    if opcion == "1":
        while True:
            tablero = []
            print(
                "Introduce el tablero de Sudoku, fila por fila, usando 0 para los huecos."
            )
            ...
            for i in range(9):
                while True:
                    ...
                    if len(fila_input) != 9 or not fila_input.isdigit():
                        print(
                            f"Error: La fila {i+1} no es válida. Debe tener exactamente 9 dígitos."
                        )
                        continue
            ...
            while correcto not in ["s", "n"]:
                ...
                correcto = (
                    input("\n ¿El tablero introducido es correcto? (s/n): ")
                    .strip()
                    .lower()
                )
        ...
        while True:
            cadena_input = input(
                'Introduce el tablero como una cadena de 81 caracteres ("." para huecos):'
            ).strip()
            ...
            tablero = [
                [int(cadena_input[i * 9 + j]) for j in range(9)] for i in range(9)
            ]
    ...
    for bloque_fila in range(3):
        for bloque_col in range(3):
            ...
            if len(bloque) != len(set(bloque)):
                print(
                    f"Error: Número repetido en bloque ({bloque_fila+1},{bloque_col+1})."
                )
                return False
    return True
    ...
    for fila in range(9):
        for col in range(9):
            if tablero[fila][col] == 0 and len(candidatos[fila][col]) == 1:
                ...
                imprimir_candidatos(
                    candidatos,
                    f"Candidato único asignado en fila {fila+1}, columna {col+1}",
                )
...
def aplicar_pares_desnudos(tablero, candidatos):
    ...
    # Aplicar en filas
    for fila in range(9):
        cambio_realizado |= buscar_pares_en_unidad(
            [candidatos[fila][col] for col in range(9)], f"Fila {fila+1}"
        )

    # Aplicar en columnas
    for col in range(9):
        cambio_realizado |= buscar_pares_en_unidad(
            [candidatos[fila][col] for fila in range(9)], f"Columna {col+1}"
        )
    ...
    for bloque_fila in range(3):
        for bloque_col in range(3):
            ...
            cambio_realizado |= buscar_pares_en_unidad(
                celdas, f"Bloque ({bloque_fila+1},{bloque_col+1})"
            )
...
def aplicar_conjuntos_desnudos(tablero, candidatos, tamaño):
    ...
    for fila in range(9):
        cambio_realizado |= buscar_conjuntos_desnudos_en_unidad(
            [candidatos[fila][col] for col in range(9)], tamaño, f"Fila {fila+1}"
        )

    # Columnas
    for col in range(9):
        cambio_realizado |= buscar_conjuntos_desnudos_en_unidad(
            [candidatos[fila][col] for fila in range(9)], tamaño, f"Columna {col+1}"
        )
        ...
        for bloque_col in range(3):
            ...
            for r in range(3):
                for c in range(3):
                    ...
                    celdas.append(candidatos[fila][col])
            cambio_realizado |= buscar_conjuntos_desnudos_en_unidad(
                celdas, tamaño, f"Bloque ({bloque_fila+1},{bloque_col+1})"
            )
...
def buscar_conjuntos_desnudos_en_unidad(celdas, tamaño, nombre_unidad):
    ...
    for combo in combinations(indices, tamaño):
        ...
        if len(conjunto) == tamaño:
            ...
            if cambio:
                print(
                    f"{tamaño}-desnudo encontrado en {nombre_unidad}: {sorted(conjunto)}"
                )
    return cambio
...
def aplicar_singles_ocultos(tablero, candidatos):
    ...
    for fila in range(9):
        cambio_realizado |= buscar_singles_ocultos_en_unidad(
            tablero, candidatos, [(fila, col) for col in range(9)], f"Fila {fila+1}"
        )

    # Columnas
    for col in range(9):
        cambio_realizado |= buscar_singles_ocultos_en_unidad(
            tablero, candidatos, [(fila, col) for fila in range(9)], f"Columna {col+1}"
        )
    ...
    for bloque_fila in range(3):
        for bloque_col in range(3):
            ...
            cambio_realizado |= buscar_singles_ocultos_en_unidad(
                tablero, candidatos, celdas, f"Bloque ({bloque_fila+1},{bloque_col+1})"
            )
...
def buscar_singles_ocultos_en_unidad(tablero, candidatos, celdas, nombre_unidad):
    ...
    for candidato, posiciones in candidato_a_indices.items():
        if len(posiciones) == 1:
            ...
            print(
                f"Single oculto encontrado en {nombre_unidad}: {candidato} en fila {fila+1}, columna {col+1}"
            )
            imprimir_tablero(tablero)
            imprimir_candidatos(
                candidatos, f"Single oculto asignado en fila {fila+1}, columna {col+1}"
            )
            cambio = True
    ...
    for i in range(len(candidatos)):
        for j in range(i + 1, len(candidatos)):
            ...
            if indices_i == indices_j and len(indices_i) == 2:
                ...
                if cambio:
                    print(
                        f"Par oculto encontrado en {nombre_unidad}: {sorted([candidatos[i], candidatos[j]])}"
                    )
    return cambio
...
def _resolver_sudoku_backtracking_recursivo(tablero, stats):
    ...
    stats["max_profundidad"] = max(
        stats["max_profundidad"], stats["profundidad_actual"]
    )
...
def main():
    ...
    tablero_original_para_bt_stats = [
        fila[:] for fila in tablero_leido
    ]  # Copia para comparar celdas llenadas
    ...
    while paso_iteracion_heuristica <= max_iteraciones_heuristicas:
        ...
        if (
            paso_iteracion_heuristica == 1
        ):  # Guardar el estado inicial antes de cualquier heurística para el cálculo de celdas llenadas por BT
            tablero_antes_de_bt_definitivo = [fila[:] for fila in tablero_actual]

        # Aplicar técnicas más directas primero
        if aplicar_candidato_unico(tablero_actual, candidatos):
            cambio_en_iteracion = True
        if aplicar_singles_ocultos(tablero_actual, candidatos):
            cambio_en_iteracion = True

        # Si las directas no hicieron cambio, probar las más complejas de eliminación
        if not cambio_en_iteracion:
            if aplicar_pares_desnudos(tablero_actual, candidatos):
                cambio_en_iteracion = True
            # Podrías re-evaluar `cambio_en_iteracion` después de cada llamada si quieres ser granular
            if aplicar_conjuntos_desnudos(tablero_actual, candidatos, 3):
                cambio_en_iteracion = True
            if aplicar_conjuntos_desnudos(tablero_actual, candidatos, 4):
                cambio_en_iteracion = True
            if aplicar_pares_ocultos(tablero_actual, candidatos):
                cambio_en_iteracion = True

        # Comprobar si el tablero está resuelto después de esta iteración de heurísticas
        resuelto_por_heuristica_en_iteracion = all(
            tablero_actual[r][c] != 0 for r in range(9) for c in range(9)
        )
    ...
    resuelto_final_heuristicas = all(
        tablero_actual[r][c] != 0 for r in range(9) for c in range(9)
    )
    ...
    if not resuelto_final_heuristicas:
        ...
        imprimir_tablero(
            tablero_antes_de_bt_definitivo
        )  # Mostrar el tablero que ENTRARÁ al BT
        ...
        tablero_solucion_bt, iter_bt, prof_bt, tiempo_bt, retrocesos_bt = (
            resolver_sudoku_fuerza_bruta(tablero_actual)
        )
        ...
        if tablero_solucion_bt:
            ...
            for r in range(9):
                for c in range(9):
                    if (
                        tablero_antes_de_bt_definitivo[r][c] == 0
                        and tablero_solucion_bt[r][c] != 0
                    ):
                        celdas_llenadas_por_bt += 1
            print(
                f"  Celdas llenadas por backtracking: {celdas_llenadas_por_bt}"
            )  # << NUEVA ESTADÍSTICA
            ...
            print(
                "El tablero mostrado a continuación es el estado en el que quedó después de las técnicas heurísticas (antes del intento de backtracking):"
            )
            imprimir_tablero(
                tablero_antes_de_bt_definitivo
            )  # tablero_actual no fue modificado por el BT fallido
